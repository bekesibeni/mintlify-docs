---
title: "Data models"
description: "TypeScript types for every shape returned by the API. Generated equivalents are available via openapi-typescript or the official SDK."
---

These types mirror the OpenAPI `data` shapes — what's inside `body.data` on a
successful envelope. Generate them with `openapi-typescript`
([TypeScript types guide](/guides/sdk)) or import from
`@skinshark/sdk/types`.
...
// /merchant/trades, /merchant/users/{id}/trades, /market/trades
...
interface PayoutQuoteResponse {
  ...
  token: string;
  amountCents: string;
  feeMultiplier: number;            // 1.5 by default; admin-configurable
  liveFeeUsdCents: string;          // live gas × multiplier
...
For exhaustive type coverage, generate types from the OpenAPI spec or use the
