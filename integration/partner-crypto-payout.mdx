...
| `payout.crypto.withdraw.failed` | Approval rejected or broadcast permanently failed | `data.withdrawal` with `failureReason` |
| `payout.crypto.withdraw.refunded` | Sidecar re-credited; paired with `failed` | `data.withdrawal` |
