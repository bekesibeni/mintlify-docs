...
- `payout.crypto.withdraw.failed` — terminally failed (approval rejected, or broadcast permanently failed).
- `payout.crypto.withdraw.refunded` — sidecar re-credited, payout wallet restored. Always paired with `failed` — partners can rely on either to know funds returned.

## The approval callback — synchronous, not queued

`payout.crypto.withdraw.approval` is **fundamentally different** from every other event in this guide:
...
| Signature scheme | Same Standard Webhooks signing (using your active webhook secret) | Same |
...
2. Look up `externalId` in your records. If you didn't create this withdrawal — respond **4xx immediately** to refund.
3. If you did create it, confirm the destination, amount, and `forUserExternalId` match what you expect.
4. Return 2xx within 5 seconds → the worker proceeds to `queued` → `broadcast`.
...
The async lifecycle events (`payout.crypto.withdraw.broadcast`, `.confirmed`, `.failed`, `.refunded`) carry a `withdrawal` object instead — same shape as `GET /payout/crypto/withdrawals/{id}` plus the resolved `forUserExternalId`.
...
interface WebhookEnvelope {
  ...
  data:
    ...
    | {
      ...
      };                                 // payout.crypto.withdraw.{broadcast,confirmed,failed,refunded}
}
```

The synchronous **approval** callback (`payout.crypto.withdraw.approval`) is a different
shape entirely — see the section above for its payload.
